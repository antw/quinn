...
b.prototype.__setValue=function(a,c){var e=this.value,d,b;if(this.isRange){if(g.isArray(a)){if(this.value!=null){if(e=g.clone(e),a.length!=e.length)return false}else this.value=[];a=g.clone(a);for(d=0,b=a.length;d<b;d++)a[d]=this.__sanitizeValue(a[d])}else if(g.isNumber(a)){if(!this.activeHandle)return false;a=this.__sanitizeValue(a);this.activeHandle.index===1?a<=this.handles[0].value&&(a=this.handles[0].value+this.options.step):a>=this.handles[1].value&&(a=this.handles[1].value-this.options.step);
d=a;a=g.clone(this.value);a[this.activeHandle.index]=d}else a=g.clone(this.selectable);if(g.isEqual(e,a))return false;this.value=a}else{a=g.isNumber(a)?[this.__sanitizeValue(a)]:[this.selectable[0]];if(e===a[0])return false;this.value=a[0]}if(!this.trigger("drag",this.value))return this.value=e,false;for(d=0,b=this.handles.length;d<b;d++)this.handles[d].value=a[d];this.sendRenderer("redraw",c);return true};b.Renderer=function(a,c){this.quinn=a;this.wrapper=a.wrapper;this.options=c};b.Renderer.prototype.render=
