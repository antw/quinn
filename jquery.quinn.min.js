(function(f,d){function b(a,c){d.bindAll(this,"clickBar","startDrag","drag","endDrag");this.wrapper=a;this.options=d.extend({},b.defaults,c);this.callbacks={};this.disabled=false;this.previousValue=this.activeHandle=null;this.model=new i(this);this.renderer=new this.options.renderer(this);this.leftExtent=this.options.range[0];this.rightExtent=this.options.range[1];this.on("setup",this.options.onSetup);this.on("begin",this.options.onBegin);this.on("drag",this.options.onDrag);this.on("change",this.options.onChange);
this.on("abort",this.options.onAbort);d.isFunction(this.renderer.render)&&this.renderer.render();this.options.disable===true&&this.disable();this.trigger("setup",this.model.value)}function i(a){var c,e;this.options=a.options;this.values=[];this.previousValues=[];this.step=a.options.step;this.only=a.options.only;a=this.options.selectable||this.options.range;this.minimum=a[0];this.maximum=a[1];c=this.roundToStep(a[0]);e=this.roundToStep(a[1]);c<a[0]&&(c+=this.options.step);e>a[1]&&(e-=this.options.step);
this.minimum=c;this.maximum=e;a=this.options.value==null?this.minimum:d.isArray(this.options.value)?this.options.value:[this.options.value];for(e=0,c=a.length;e<c;e++)this.values[e]=null;this.setValue(a)}var k="mousemove",m="mousedown",l="mouseup",h=false;"ontouchstart"in document.documentElement&&(k="touchmove",m="touchstart",l="touchend",h=true);b.prototype.on=function(a,c){d.isString(a)&&d.isFunction(c)&&(this.callbacks[a]||(this.callbacks[a]=[]),this.callbacks[a].push(c));return this};b.prototype.trigger=
function(a,c){var e=this.callbacks[a]||[],b,d=0;if(c===void 0)c=this.value;for(;b=e[d++];)if(b(c,this)===false)return false;return true};b.prototype.enable=function(){if(this.disabled)this.disabled=false,this.trigger("enabled")};b.prototype.disable=function(){if(!this.disabled)this.disabled=true,this.trigger("disabled")};b.prototype.setValue=function(a,c,e){this.willChange()&&(this.setTentativeValue(a,c,e)!==false?this.hasChanged():this.abortChange());return this.model.value};b.prototype.setTentativeValue=
function(a,c,e){var b=this.model.value,f,g,j;if(a==null)return false;if(this.model.values.length>1&&d.isNumber(a))if(this.activeHandle!=null)f=this.model.sanitizeValue(a),a=d.clone(this.model.values),g=a[this.activeHandle-1],j=a[this.activeHandle+1],g!=null&&f<=g&&(f=g+this.options.step),j!=null&&f>=j&&(f=j-this.options.step),a[this.activeHandle]=f;else return false;a=this.model.setValue(a);if(a===false||!e&&!this.trigger("drag",a))return this.model.setValue(b),false;this.trigger("redraw",c);return a};
b.prototype.stepUp=function(a){return this.model.values.length>1?this.model.value:this.setValue(this.model.value+this.options.step*(a||1))};b.prototype.stepDown=function(a){return this.stepUp(-(a||1))};b.prototype.willChange=function(){if(this.disabled===true||!this.trigger("begin"))return false;this.previousValue=this.model.value;return true};b.prototype.hasChanged=function(){this.deactivateActiveHandle();if(!this.trigger("change",this.model.value))return this.setTentativeValue(this.previousValue),
...
1)return true;if(!c&&!this.willChange())return false;this.activateHandleWithEvent(a);f(document).on(l+".quinn",this.endDrag).on(k+".quinn",this.drag).on("mouseenter.quinn",this.endDrag);return false};b.prototype.drag=function(){var a=event.pageX;if(event.type==="touchmove")a=event.originalEvent.targetTouches[0].pageX;this.setTentativeValue(this.valueFromMouse(a),false);return event.preventDefault()};b.prototype.endDrag=function(a){f(document).off(l+".quinn").off(k+".quinn").off("mouseenter.quinn");
