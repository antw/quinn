...
f=d.min,e=d.max,h=h||{};this.drawTo={left:g.has(h,"left")?h.left:f,right:g.has(h,"right")?h.right:e};this.model=new j(this,this.options.strict);this.renderer=new this.options.renderer(this);this.wrapperOffset=this.wrapperWidth=0;this.on("setup",d.setup);this.on("begin",d.begin);this.on("drag",d.drag);this.on("change",d.change);this.on("abort",d.abort);g.isFunction(this.renderer.render)&&this.renderer.render();d.disable===true&&this.disable();this.trigger("setup",this.model.value)}function j(a,b){var d,
h,c;this.options=d=a.options;this.step=d.step;this.only=d.only;this.values=[];this.minimum=this.roundToStep(d.min);this.maximum=this.roundToStep(d.max);this.minimum<d.min&&(this.minimum+=this.step);this.maximum>d.max&&(this.maximum-=this.step);d=typeof d.value==="undefined"||d.value===null?this.minimum:g.isArray(d.value)?d.value:[d.value];for(c=0,h=d.length;c<h;c++)this.values[c]=null;this.setValue(d,b)}var l="mousemove",m="mousedown",n="mouseup",o=false;"ontouchstart"in document.documentElement&&
(l="touchmove",m="touchstart",n="touchend",o=true);c.VERSION="1.0.2";c.prototype.on=function(a,b){g.isString(a)&&g.isFunction(b)&&(this.callbacks[a]||(this.callbacks[a]=[]),this.callbacks[a].push(b));return this};c.prototype.trigger=function(a,b){var d=this.callbacks[a]||[],c=0,f;if(b===void 0)b=this.value;for(;f=d[c++];)if(f(b,this)===false)return false;return true};c.prototype.enable=function(){if(this.disabled)this.disabled=false,this.trigger("enabled")};c.prototype.disable=function(){if(!this.disabled)this.disabled=
