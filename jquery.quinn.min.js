...
!1;this.previousValue=this.activeHandle=null;a.is("input")&&this.enhance(a);var g=d.drawTo,k=d.min,e=d.max,g=g||{};this.drawTo={left:f.has(g,"left")?g.left:k,right:f.has(g,"right")?g.right:e};this.model=new m(this,this.options.strict);this.renderer=new this.options.renderer(this);this.keyFloodTimer=null;this.wrapperOffset=this.wrapperWidth=0;this.on("setup",d.setup);this.on("begin",d.begin);this.on("drag",d.drag);this.on("change",d.change);this.on("abort",d.abort);this.on("resize",this.renderer.resized);
f.isFunction(this.renderer.render)&&this.renderer.render();!0===d.disable&&this.disable();this.trigger("setup",this.model.value)}function m(a,b){var d,g,c;this.options=d=a.options;this.step=d.step;this.only=d.only;this.values=[];this.minimum=this.roundToStep(d.min);this.maximum=this.roundToStep(d.max);this.minimum<d.min&&(this.minimum+=this.step);this.maximum>d.max&&(this.maximum-=this.step);d=void 0===d.value||null===d.value?this.minimum:f.isArray(d.value)?d.value:[d.value];c=0;for(g=d.length;c<
g;c++)this.values[c]=null;this.setValue(d,b)}var p="mousemove",q="mousedown",r="mouseup",s=!1;"ontouchstart"in document.documentElement&&(p="touchmove",q="touchstart",r="touchend",s=!0);c.VERSION="1.1.0";c.prototype.on=function(a,b){f.isString(a)&&f.isFunction(b)&&(this.callbacks[a]||(this.callbacks[a]=[]),this.callbacks[a].push(b));return this};c.prototype.trigger=function(a,b){var d=this.callbacks[a]||[],g=0,c;void 0===b&&(b=this.value);for(;c=d[g++];)if(!1===c(b,this))return!1;return!0};c.prototype.enable=
function(){this.disabled&&(this.disabled=!1,this.trigger("enabled"))};c.prototype.disable=function(){this.disabled||(this.disabled=!0,this.trigger("disabled"))};c.prototype.setValue=function(a,b,d){this.start()&&(!1!==this.setTentativeValue(a,b,d)?this.resolve():this.reject());return this.model.value};c.prototype.setTentativeValue=function(a,b,d){var c=this.model.value,k=null,e=null,h;if(void 0===a||null===a)return!1;if(1<this.model.values.length&&f.isNumber(a)){if(null===this.activeHandle)return!1;
h=this.model.sanitizeValue(a);a=f.clone(this.model.values);k=a[this.activeHandle-1];e=a[this.activeHandle+1];null!==k&&h<=k&&(h=k+this.options.step);null!==e&&h>=e&&(h=e-this.options.step);a[this.activeHandle]=h}a=this.model.setValue(a);if(!1===a||!d&&!this.trigger("drag",a))return this.model.setValue(c),!1!==a&&this.trigger("drag",c),!1;this.trigger("redraw",b);return a};c.prototype.stepUp=function(a,b,d){return 1<this.model.values.length?this.model.value:f.bind(d?this.setTentativeValue:this.setValue,
this)(this.model.value+this.model.step*(a||1),b)};c.prototype.stepDown=function(a,b,d){return this.stepUp(-(a||1),b,d)};c.prototype.start=function(){if(!0===this.disabled||!this.trigger("begin"))return!1;this.previousValue=this.model.value;this.wrapperWidth=this.wrapper.width();this.wrapperOffset=this.wrapper.offset().left;return!0};c.prototype.resolve=function(){this.deactivateActiveHandle();if(f.isEqual(this.previousValue,this.model.value))return this.reject(),!1;if(!this.trigger("change",this.model.value))return this.setTentativeValue(this.previousValue),
this.reject(),!1;this.previousValue=null};c.prototype.reject=function(){this.previousValue=null;this.deactivateActiveHandle();return this.trigger("abort")};c.prototype.valueFromMouse=function(a){a=this.positionFromMouse(a);return this.drawTo.left+(this.drawTo.right-this.drawTo.left)*a};c.prototype.positionFromMouse=function(a){var b=this.wrapperOffset+this.wrapperWidth;return(a<this.wrapperOffset?0:a>b?this.wrapperWidth:a-this.wrapperOffset)/this.wrapperWidth};c.prototype.startDrag=function(a,b){if(!s&&
...
this))};m.prototype.setValue=function(a,b){var d=this.values,c,e;a=f.isArray(a)?f.clone(a):[a];e=0;for(c=a.length;e<c;e++)a[e]=this.sanitizeValue(a[e],b);if(f.isEqual(a,d))return!1;this.value=this.values=a;1===this.values.length&&(this.value=a[0]);return this.value};m.prototype.roundToStep=function(a){var b=1/this.step,b=Math.round(a*b)/b;f.isArray(this.only)&&(b=f.min(this.only,function(b){return Math.abs(b-a)}));return b>this.maximum?b-this.step:b<this.minimum?b+this.step:b};m.prototype.sanitizeValue=
function(a,b){!1!==b&&(a=this.roundToStep(a));return a>this.maximum?this.maximum:a<this.minimum?this.minimum:a};c.Renderer=function(a){f.bindAll(this,"render","redraw","resized");var b=this;this.quinn=a;this.model=a.model;this.wrapper=a.wrapper;this.options=a.options;this.handles=[];this.lastDraw=[];a.on("redraw",this.redraw);a.on("handleOn",function(a){b.handles[a].addClass("active")});a.on("handleOff",function(a){b.handles[a].removeClass("active")});a.on("enabled",function(){b.wrapper.removeClass("disabled");
...
this.wrapper.on(q,this.quinn.clickBar);this.barHeight=this.bar.height();this.handleOverhang=parseInt(this.handles[0].css("margin-left"),10)+parseInt(this.handles[0].css("margin-right"),10);this.redraw(!1)};c.Renderer.prototype.redraw=function(a,b){var d=this;!1!==a&&(a=!0);f.each(this.model.values,function(c,e){var f,h;if(!b&&c===d.lastDraw[e])return!0;f=d.handles[e].stop();h=d.position(c)+"px";a&&d.options.effects?f.animate({left:h},{duration:d.options.effectSpeed,step:d.redrawDeltaBarInStep(f)}):
(f.css("left",h),d.redrawDeltaBar(c,f))});this.lastDraw=f.clone(this.model.values)};c.Renderer.prototype.resized=function(){this.width=this.wrapper.width();this.redraw(!1,!0)};c.Renderer.prototype.redrawDeltaBar=function(a,b){var d=null,c=null,e=parseInt(b.position().left,10)+this.barHeight;this.deltaBar.stop(!0);1<this.model.values.length?b?b===this.handles[0]?d=e:c=e:(d=a[0],c=a[1]):0>a?(d=e,c=this.position(0,!0)):(c=e,d=this.position(0,!0));null!==d&&this.deltaBar.css("left",d);null!==c&&(c=this.width-
...
d?0:d>c?c:Math.round(d)};c.defaults={min:0,max:100,drawTo:null,step:1,value:null,strict:!0,only:null,disable:!1,disabledOpacity:0.5,width:null,handleWidth:null,drag:null,change:null,setup:null,renderer:c.Renderer,keyFloodWait:!1,effectSpeed:"fast",effects:!0};e.fn.quinn=function(a){return e.each(this,function(){e(this).data("quinn",new c(e(this),a))})};e.Quinn=c})(jQuery,_);
