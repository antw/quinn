...
e<this.selectable[0]&&(e+=this.options.step);d>this.selectable[1]&&(d-=this.options.step);this.selectable=[e,d];this.wrapper.data("quinn",this);if(this.isRange){this.wrapper.addClass("range");for(d=0,e=this.options.value.length;d<e;d++)this.handles.push({id:d,value:this.options.value[d],element:null})}else this.handles.push({id:0,value:this.options.value,element:null});this.renderer=new this.options.renderer(this,this.options);this.sendRenderer("render");this.bar=f(".bar",this.wrapper);this.__setValue(this.options.value,
...
true;this.wrapper.addClass("disabled");this.options.disabledOpacity!==1&&this.wrapper.css("opacity",this.options.disabledOpacity)};b.prototype.enable=function(){this.isDisabled=false;this.wrapper.removeClass("disabled");this.options.disabledOpacity!==1&&this.wrapper.css("opacity",1)};b.prototype.bind=function(a,c){g.isString(a)&&g.isFunction(c)&&(a.slice(0,2)==="on"&&(a=a.slice(2,a.length).toLowerCase()),this.callbacks[a].push(c));return this};b.prototype.trigger=function(a,c){var e=this.callbacks[a],
d,b=0;if(c===void 0)c=this.value;for(;d=e[b++];)if(d(c,this)===false)return false;return true};b.prototype.clickBar=function(a){if(!h&&a.which!==1)return true;this.__willChange()&&(this.__activateHandleWithEvent(a),this.__setValue(this.__valueFromMouse(a.pageX),true),h?this.__hasChanged():this.enableDrag(a,true));return a.preventDefault()};b.prototype.enableDrag=function(a,c){if(!h&&a.which!==1)return true;if(!c&&!this.__willChange())return false;this.__activateHandleWithEvent(a);f(document).bind(j+
".quinn",this.disableDrag).bind(i+".quinn",this.drag).bind("mouseenter.quinn",this.disableDrag);return a.preventDefault()};b.prototype.disableDrag=function(a){f(document).unbind(j+".quinn").unbind(i+".quinn").unbind("mouseenter.quinn");this.activeHandle.element.removeClass("active");this.__hasChanged();return a.preventDefault()};b.prototype.drag=function(a){var c=a.pageX;if(a.type==="touchmove")c=a.originalEvent.targetTouches[0].pageX;this.__setValue(this.__valueFromMouse(c));return a.preventDefault()};
