...
a.append(d('<div class="main" />'));a.append(d('<div class="right" />'))}var b,c;this.width=this.options.width||this.wrapper.width();this.bar=d('<div class="bar" />');this.activeBar=d('<div class="active-bar" />');this.model.values.length>1&&this.wrapper.addClass("range");a(this.bar);a(this.activeBar);this.model.values.length<=2&&this.bar.append(this.activeBar);this.wrapper.html(this.bar);this.wrapper.addClass("quinn");for(b=0,c=this.model.values.length;b<c;b++)this.handles[b]=d('<span class="handle"></span>'),
this.handles[b].on(n,this.quinn.startDrag),this.bar.append(this.handles[b]);this.bar.css({width:this.width.toString()+"px"});this.wrapper.on("mousedown",this.quinn.clickBar);this.redraw(false)};c.Renderer.prototype.redraw=function(a){var b=this.options,c=this.quinn.leftExtent,d=this.quinn.rightExtent,i=d-c;a==void 0&&(a=true);this.activeBar.stop(true);e.each(this.handles,e.bind(function(g,j){var h;g.stop(true);h=((this.width-5)*((this.model.values[j]-c)/i)).toString()+"px";a&&b.effects?g.animate({left:h},
{duration:b.effectSpeed,step:e.bind(function(a){a/=this.width;this.redrawActiveBar(a*(d-c)+c,g);return true},this)}):(g.css("left",h),this.redrawActiveBar(this.model.value))},this))};c.Renderer.prototype.redrawActiveBar=function(a,b){var c=null,d=null;this.activeBar.stop();this.model.values.length>1?b?b===this.handles[0]?c=this.__positionForValue(a):d=this.__positionForValue(a):(c=this.__positionForValue(a[0]),d=this.__positionForValue(a[1])):a<0?(c=this.__positionForValue(a),d=this.__positionForValue(0)):
(c=this.__positionForValue(0),d=this.__positionForValue(a));d=this.bar.width()-d;c!==null&&this.activeBar.css("left",c.toString()+"px");d!==null&&this.activeBar.css("right",d.toString()+"px")};c.Renderer.prototype.__positionForValue=function(a){a=(a-this.quinn.leftExtent)/(this.quinn.rightExtent-this.quinn.leftExtent)*this.width;return a<0?0:a>this.width?this.width:Math.round(a)};c.defaults={range:[0,100],selectable:null,step:1,value:null,only:null,disable:false,disabledOpacity:0.5,width:null,handleWidth:null,
onDrag:null,onChange:null,onSetup:null,renderer:c.Renderer,effectSpeed:"fast",effects:true};d.fn.quinn=function(a){return d.each(this,function(){new c(d(this),a)})};d.Quinn=c})(jQuery,_);
